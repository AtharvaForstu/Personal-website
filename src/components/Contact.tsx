...
export default function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    emailjs
      .send(
        'service_atharva', // Service ID
        'template_6nc60jn', // Template ID
        {
          name: formData.name,
          email: formData.email,
          message: formData.message,
        },
        'W1Ma7_c5aDjfy0Roo' // Public key
      )
      .then(
        (result) => {
    ...
    <section id="contact" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        <div>
          ...
          <p className="text-xl text-gray-300 mb-8">Let's discuss your project</p>
        ...
        >
          ...
          {status && <p className="text-center text-sm mt-4 text-white">{status}</p>}
        </form>
      </div>
    </section>
  );
